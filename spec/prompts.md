Este arquivo é a fonte da verdade do system prompt do Assistente do Taskly — define persona, capacidades, limites e comportamento diante de ambiguidade e erros. É o principal critério de avaliação de "design de agente" desta extensão (ver `ADR-003`). O conteúdo do bloco abaixo é espelhado literalmente em `app/assistant/system_prompt.py`.
